// Adopted from: https://github.com/jamesqquick/netlify-auth0-rbac-integration-demo/blob/master/functions/AuthUtils.js
...
const generateNetlifyJWT = async (tokenData) => {
  ...
  const secretKey = createSecretKey(Buffer.from(process.env.TOKEN_SECRET, 'utf8'));
...
const handleLogin = async (event) => {
  ...
  const state = generateEncodedStateString(returnTo);
...
const handleCallback = async (event) => {
  ...
  const loginCookie = cookie.parse(event.headers.cookie)[AUTH0_LOGIN_COOKIE_NAME];
  ...
  const netlifyCookie = await generateNetlifyCookieFromAuth0Token(tokenSet.claims());
